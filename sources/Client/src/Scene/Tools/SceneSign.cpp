...
bool CSceneSign::Init(CGameScene* pScene) {
	...
	if (_pAttack) {
		...
		g_logManager.InternalLog(LogLevel::Debug, "common", GetLanguageString(377));
		return false;
	}
	...
	if (_pAttackShade) {
		...
		g_logManager.InternalLog(LogLevel::Debug, "common", GetLanguageString(378));
		return false;
	}
	...
	if (_pMove) {
		...
		g_logManager.InternalLog(LogLevel::Debug, "common", GetLanguageString(379));
		return false;
	}
	return true;
}
