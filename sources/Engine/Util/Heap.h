...
namespace Corsairs::Engine::Render {
	...
	public:
		...
		typedef BOOL (*HeapTCompareProc)(const T& i, const T& j);
		typedef void (*HeapTFilterProc)(const T& t, DWORD i);
	...
	private:
		...
		HeapTCompareProc _comp_proc;
		HeapTFilterProc _filter_proc;

	private:
		DWORD _Parent(DWORD i) {
			return (i - 1) / 2;
		}
		...
		BOOL _CheckBufferState() {
			return (_num < _buf_size);
		}
	...
	public:
		...
		LW_RESULT Copy(HeapT* src);

		void SetCompareProc(HeapTCompareProc proc) {
			_comp_proc = proc;
		}

		void SetFilterProc(HeapTFilterProc proc) {
			_filter_proc = proc;
		}

		HeapTCompareProc GetCompareProc() {
			return _comp_proc;
		}

		HeapTFilterProc GetFilterProc() {
			return _filter_proc;
		}
	...
	template <typename T>
	LW_RESULT HeapT<T>::_Allocate(DWORD buf_size) {
	...
	template <typename T>
	DWORD HeapT<T>::_FilterDown(DWORD i) {
		T t = _Value(i);
		DWORD index = i;
		DWORD l = _Left(i);
		DWORD r = _Right(i);
	...
	template <typename T>
	DWORD HeapT<T>::_FilterUp(DWORD i) {
		T t = _Value(i);
		DWORD index = i;
		DWORD p = _Parent(i);
	...
	template <typename T>
	LW_RESULT HeapT<T>::Reserve(DWORD size) {
		return _Allocate(size);
	}

	template <typename T>
	LW_RESULT HeapT<T>::Clear() {
		return _Free();
	}

	template <typename T>
	LW_RESULT HeapT<T>::Push(const T& t) {
		if (_num >= _buf_size) {
	...
	template <typename T>
	LW_RESULT HeapT<T>::Push(const T* buf, DWORD size) {
	...
	template <typename T>
	LW_RESULT HeapT<T>::Find(DWORD* id, const T& t) {
		if (_num == 0)
			return LW_RET_FAILED;
	...
	template <typename T>
	LW_RESULT HeapT<T>::Update(DWORD id) {
		if (id >= _num)
			return LW_RET_FAILED;
	...
	template <typename T>
	LW_RESULT HeapT<T>::Remove(DWORD id) {
		if (id >= _num)
			return LW_RET_FAILED;
	...
	template <typename T>
	LW_RESULT HeapT<T>::Copy(HeapT* src) {
		Clear();
	...
	public:
		...
		void SetCompareProc(HeapCompProc proc) {
			_heap.SetCompareProc((T::HeapTCompareProc)proc);
		}

		void SetFilterProc(HeapFilterProc proc) {
			_heap.SetFilterProc((T::HeapTFilterProc)proc);
		}

		HeapCompProc GetCompareProc() {
			return (HeapCompProc)_heap.GetCompareProc();
		}

		HeapFilterProc GetFilterProc() {
			return (HeapFilterProc)_heap.GetFilterProc();
		}
		...
		LW_RESULT Clone(IHeap** out_heap);
	};
