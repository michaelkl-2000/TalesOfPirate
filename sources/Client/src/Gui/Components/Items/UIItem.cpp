...
void CItemEx::renderHighlighted(int x, int y) {
	...
	while (true) {
		...
		if (vectorIndex < highlights.size() && strIndex >= highlights[vectorIndex].start && strIndex < highlights[
			vectorIndex].start + highlights[vectorIndex].len) {
	...
	if (_bParseText) {
		...
		if (!_bMultiLine) {
			if (GetHeadLength() && GetHeadLength() <= _str.length()) {
				...
				g_TextParse.Render((char*)strTail.c_str(), x + ui::GetWidth(strHead.c_str()), y, _dwColor, m_Allign,
								   _nHeight);
			}
			else {
