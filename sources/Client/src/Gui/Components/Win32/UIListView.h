...
namespace GUI {
	...
	public:
		...
		public:
			...
			virtual CImage* GetImage(int index) {
				return NULL;
			}
		...
		public:
			int GetColumnWidth(unsigned int nCol) {
				return _pList->GetList()->GetItems()->GetColumnWidth(nCol);
			}
		...
		public:
			...
			virtual bool MouseRun(int x, int y, DWORD key) {
				return _pImage->MouseRun(x, y, key);
			}
		...
		public:
			...
			virtual bool MouseRun(int x, int y, DWORD key) {
				return _pColumn->MouseRun(x, y, key);
			}

			virtual void SetParent(CListView* p) {
				_pColumn->SetParent(p);
			}

			virtual CImage* GetImage(int index) {
				return dynamic_cast<CImage*>(_pColumn->GetIndex(index));
			}

			virtual void SetAlpha(BYTE alpha) {
				_pColumn->SetAlpha(alpha);
			}
	...
	public:
		...
		CImage* GetColumnImage(int index) {
			return _pTitle->GetImage(index);
		}

		void SetColumnHeight(unsigned int v) {
			_nColumnHeight = v;
		}

		unsigned int GetColumnHeight() {
			return _nColumnHeight;
		}
	...
	private:
		void _ColumnClick(CGuiData* pSender) {
			if (evtColumnClick) evtColumnClick(pSender);
		}
