...
public:
	...
	void SetSessionFormPointer(void* p) {
		_pPointer = p;
	}

	void* GetSessionFormPointer() {
		return _pPointer;
	}
...
public:
	...
	void AddMemberToBuffer(std::string name) {
		m_strWaitForAdded += name + "\n";
	}
...
public:
	...
	static CTalkSessionForm*
	CreateHiddenSessionForm(DWORD sessionID, stNetSessCreate* pArrMembers[], int memberNum = 1);
