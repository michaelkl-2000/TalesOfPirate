...
public:
	...
	CSceneObj* GetSelectSceneObj() {
		return _pSelSceneObj;
	}
...
protected:
	...
	int _GetSceneObjBlockInfo(MPSceneObject* obj, int* x, int* y, int* width, int* height, BYTE** block_seq,
							  float** height_seq);
...
public:
	...
	void enable_erase(bool erase = true) {
		_is_erase = erase;
	}
};
