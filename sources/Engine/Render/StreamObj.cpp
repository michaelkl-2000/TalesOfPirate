...
namespace Corsairs::Engine::Render {
	LW_RESULT StreamObject::Init(DWORD buffer_size) {
	...
	LW_RESULT StreamObject::ReserveRoom(DWORD size, DWORD stride) {
	...
	LW_RESULT StreamObject::BindData(DWORD* out_off_addr, DWORD size, DWORD stride) {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT StreamObject::UnbindData(DWORD size, DWORD stride) {
		_free_size += (size + stride);
		return LW_RET_OK;
	}

	LW_RESULT StreamObject::ApplyRoom(DWORD* out_off_addr, DWORD size, DWORD stride) {
		LW_RESULT ret = LW_RET_FAILED;
	...
	// StreamObjVB
	StreamObjVB::StreamObjVB(IDeviceObject* dev_obj)
		: _dev_obj(dev_obj), _buf(0), _fvf(0) {
	}

	StreamObjVB::~StreamObjVB() {
		LoseDevice();
	}

	LW_RESULT StreamObjVB::Create(DWORD size, DWORD fvf) {
		LW_RESULT ret = LW_RET_FAILED;

		if (LW_RESULT r = StreamObject::Init(size); LW_FAILED(r)) {
			ToLogService("errors", LogLevel::Error,
						 "[{}] StreamObject::Init failed: size={}, ret={}",
	...
	LW_RESULT StreamObjVB::LoseDevice() {
		...
		// Reinit (como j fazia)
		if (LW_RESULT r = StreamObject::Init(_total_size); LW_FAILED(r)) {
			ToLogService("errors", LogLevel::Error,
						 "[{}] StreamObject::Init failed: total_size={}, ret={}",
	...
	LW_RESULT StreamObjVB::ResetDevice() {
	...
	LW_RESULT StreamObjVB::BindData(DWORD* out_off_addr, void* data, DWORD size, DWORD stride) {
		LW_RESULT ret = LW_RET_FAILED;

		if (LW_RESULT r = StreamObject::BindData(out_off_addr, size, stride); LW_FAILED(r)) {
			ToLogService("errors", LogLevel::Error,
						 "[{}] StreamObject::BindData(VB) failed: size={}, stride={}, ret={}",
	...
	// StreamObjIB
	StreamObjIB::StreamObjIB(IDeviceObject* dev_obj)
		: _dev_obj(dev_obj), _buf(0), _format(D3DFMT_INDEX16) {
	}

	StreamObjIB::~StreamObjIB() {
		LoseDevice();
	}

	LW_RESULT StreamObjIB::Create(DWORD size, DWORD format) {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT StreamObjIB::LoseDevice() {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT StreamObjIB::ResetDevice() {
		DWORD usage = D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY;

		return _dev_obj->CreateIndexBuffer(_total_size, usage, (D3DFORMAT)_format, D3DPOOL_DEFAULT, &_buf, LW_NULL);
	}

	LW_RESULT StreamObjIB::BindData(DWORD* out_off_addr, void* data, DWORD size, DWORD stride) {
		LW_RESULT ret = LW_RET_FAILED;

		if (LW_RESULT r = StreamObject::BindData(out_off_addr, size, stride); LW_FAILED(r)) {
			ToLogService("errors", LogLevel::Error,
						 "[{}] StreamObject::BindData(IB) failed: size={}, stride={}, ret={}",
						 __FUNCTION__, size, stride, static_cast<long long>(r));
			goto __ret;
		}
	...
	LW_STD_IMPLEMENTATION(StaticStreamMgr)

	StaticStreamMgr::StaticStreamMgr(IDeviceObject* dev_obj)
		: _dev_obj(dev_obj) {
	...
	StaticStreamMgr::~StaticStreamMgr() {
		Destroy();
	}

	LW_RESULT StaticStreamMgr::Destroy() {
	...
	LW_RESULT StaticStreamMgr::CreateStreamEntitySeq(DWORD entity_vb_size, DWORD entity_ib_size) {
		_entity_vb_seq = LW_NEW(StreamEntity[entity_vb_size]);
		_entity_vb_num = 0;
		_entity_vb_seq_size = entity_vb_size;

		_entity_ib_seq = LW_NEW(StreamEntity[entity_ib_size]);
		_entity_ib_num = 0;
	...
	LW_RESULT StaticStreamMgr::CreateVertexBufferStream(DWORD stream_id, DWORD stream_size) {
		LW_RESULT ret = LW_RET_FAILED;

		if (_stream_vb_seq[stream_id])
			goto __ret;
		{
			StreamObjVB* s = LW_NEW(StreamObjVB(_dev_obj));
	...
	LW_RESULT StaticStreamMgr::CreateIndexBufferStream(DWORD stream_id, DWORD stream_size) {
		LW_RESULT ret = LW_RET_FAILED;

		if (_stream_ib_seq[stream_id])
			goto __ret;
		{
			StreamObjIB* s = LW_NEW(StreamObjIB(_dev_obj));
	...
	LW_RESULT StaticStreamMgr::RegisterVertexBuffer(LW_HANDLE* out_handle, void* data, DWORD size, DWORD stride) {
		LW_RESULT ret = LW_RET_FAILED;
			...
			goto __ret;
		{
			...
			}
			{
				...
				StreamEntity* e = &_entity_vb_seq[this_id];
	...
	LW_RESULT StaticStreamMgr::RegisterIndexBuffer(LW_HANDLE* out_handle, void* data, DWORD size, DWORD stride) {
		LW_RESULT ret = LW_RET_FAILED;
			...
			goto __ret;
		{
			...
			}
			{
				...
				StreamEntity* e = &_entity_ib_seq[this_id];
	...
	LW_RESULT StaticStreamMgr::UnregisterVertexBuffer(LW_HANDLE handle) {
		LW_RESULT ret = LW_RET_FAILED;

		if (handle >= _entity_vb_seq_size)
			goto __ret;
		{
			StreamEntity* e = &_entity_vb_seq[handle];
			StreamObject* s = _stream_vb_seq[e->stream_id];
	...
	LW_RESULT StaticStreamMgr::UnregisterIndexBuffer(LW_HANDLE handle) {
		LW_RESULT ret = LW_RET_FAILED;

		if (handle >= _entity_ib_seq_size)
			goto __ret;
		{
			StreamEntity* e = &_entity_ib_seq[handle];
			StreamObject* s = _stream_ib_seq[e->stream_id];
	...
	LW_RESULT StaticStreamMgr::BindVertexBuffer(LW_HANDLE handle, UINT channel) {
		LW_RESULT ret = LW_RET_FAILED;
		...
		{
			StreamEntity* e = &_entity_vb_seq[handle];
			StreamObjVB* s = _stream_vb_seq[e->stream_id];
	...
	LW_RESULT StaticStreamMgr::BindIndexBuffer(LW_HANDLE handle) {
		LW_RESULT ret = LW_RET_FAILED;
		...
		{
			StreamEntity* e = &_entity_ib_seq[handle];
			StreamObjIB* s = _stream_ib_seq[e->stream_id];
	...
	LW_RESULT StaticStreamMgr::LoseDevice() {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT StaticStreamMgr::ResetDevice() {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT StaticStreamMgr::GetDebugInfo(StaticStreamMgrDebugInfo* info) {
		DWORD size = 0;
		StreamObject* s;

		memset(info, 0, sizeof(StaticStreamMgrDebugInfo));
	...
	LW_RESULT DynamicStream::GetEntryOffset(DWORD* offset, DWORD size, DWORD stride, int* fail_branch) {
	...
	LW_RESULT DynamicStream::ResetFreeSize(DWORD size, DWORD offset) {
		_free_size -= size;
	...
	// DynamicStreamVB
	DynamicStreamVB::DynamicStreamVB(IDeviceObject* dev_obj)
		: _dev_obj(dev_obj), _buf(0) {
	}

	DynamicStreamVB::~DynamicStreamVB() {
		if (_buf) // s chama se ainda existir
			LoseDevice();
	}

	LW_RESULT DynamicStreamVB::Create(DWORD buf_size, D3DFORMAT fmt) {
	...
	LW_RESULT DynamicStreamVB::LoseDevice() {
		if (_buf) {
			IDirect3DVertexBufferX* vb = _buf;
			_buf = 0;
	...
	LW_RESULT DynamicStreamVB::ResetDevice() {
		DWORD usage = D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY;
		return _dev_obj->CreateVertexBuffer(_total_size, usage, _fvf, D3DPOOL_DEFAULT, &_buf, LW_NULL);
	}

	LW_RESULT DynamicStreamVB::Bind(DWORD channel, const void* data, DWORD size, DWORD stride) {
		DWORD offset = 0;
	...
	// DynamicStreamIB
	DynamicStreamIB::DynamicStreamIB(IDeviceObject* dev_obj)
		: _dev_obj(dev_obj), _buf(0) {
	}

	DynamicStreamIB::~DynamicStreamIB() {
		if (_buf)
			LoseDevice();
	}

	LW_RESULT DynamicStreamIB::Create(DWORD buf_size, D3DFORMAT fmt) {
		_total_size = buf_size;
		_free_size = _total_size;
		_free_addr = 0;
		_fvf = fmt;
	...
	LW_RESULT DynamicStreamIB::LoseDevice() {
		if (_buf) {
	...
	LW_RESULT DynamicStreamIB::ResetDevice() {
		DWORD usage = D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY;
		return _dev_obj->CreateIndexBuffer(_total_size, usage, (D3DFORMAT)_fvf, D3DPOOL_DEFAULT, &_buf, LW_NULL);
	}

	LW_RESULT DynamicStreamIB::Bind(DWORD vert_index, const void* data, DWORD size, DWORD stride) {
		DWORD offset = 0;
		DWORD lock_flag = D3DLOCK_NOOVERWRITE;
		D3DLOCK_TYPE* lock_buf = 0;

		int fail_branch = 0;
		//  Wrap кольцевого IB — см. комментарий в DynamicStreamVB::Bind.
		const DWORD pre_free_addr = _free_addr;
		const DWORD pre_free_size = _free_size;
	...
	// DynamicStreamMgr
	LW_STD_IMPLEMENTATION(DynamicStreamMgr)

	DWORD DynamicStreamMgr::_GetVerticesSize(D3DPRIMITIVETYPE pt_type, DWORD count, DWORD stride) {
		DWORD size = 0;
	...
	DWORD DynamicStreamMgr::_GetIndicesSize(D3DPRIMITIVETYPE pt_type, DWORD count, DWORD stride) {
		DWORD size = 0;
	...
	LW_RESULT DynamicStreamMgr::Create(DWORD vb_size, DWORD ib_size) {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT DynamicStreamMgr::DrawPrimitive(D3DPRIMITIVETYPE pt_type, UINT count, const void* vert_data,
	...
	LW_RESULT DynamicStreamMgr::DrawIndexedPrimitive(D3DPRIMITIVETYPE pt_type, UINT min_vert_index,
	...
	LW_RESULT DynamicStreamMgr::BindDataVB(DWORD channel, const void* data, DWORD size, UINT stride) {
		return _vb.Bind(channel, data, size, stride);
	}

	LW_RESULT DynamicStreamMgr::BindDataIB(const void* data, DWORD size, DWORD stride) {
		return _ib.Bind(_vb.GetBaseIndex(), data, size, stride);
	}

	LW_RESULT DynamicStreamMgr::DrawPrimitive(D3DPRIMITIVETYPE pt_type, UINT start_vert, UINT count) {
		return _dev_obj->DrawPrimitive(pt_type, _vb.GetBaseIndex() + start_vert, count);
	}

	LW_RESULT DynamicStreamMgr::DrawIndexedPrimitive(D3DPRIMITIVETYPE pt_type, INT base_vert_index, UINT min_index,
	...
	LW_RESULT DynamicStreamMgr::LoseDevice() {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT DynamicStreamMgr::ResetDevice() {
		LW_RESULT ret = LW_RET_FAILED;
	...
	// LockableStreamVB
	LW_STD_IMPLEMENTATION(LockableStreamVB)

	LW_RESULT LockableStreamVB::Create(void* data, DWORD size, DWORD usage, DWORD fvf) {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT LockableStreamVB::Destroy() {
		Reset();
		LoseDevice();
	...
	LW_RESULT LockableStreamVB::Lock(UINT offset, UINT size, void** data, DWORD flag) {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT LockableStreamVB::Unlock() {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT LockableStreamVB::Bind(UINT channel, UINT offset_byte, UINT stride) {
		return _dev_obj->SetStreamSource(channel, _buf, offset_byte, stride);
	}

	LW_RESULT LockableStreamVB::LoseDevice() {
		if (_buf) {
			IDirect3DVertexBufferX* vb = _buf;
			_buf = 0;
			_dev_obj->ReleaseVertexBuffer(vb);
	...
	LW_RESULT LockableStreamVB::ResetDevice() {
		LW_RESULT ret = LW_RET_FAILED;
	...
	// LockableStreamIB
	LW_STD_IMPLEMENTATION(LockableStreamIB)

	LW_RESULT LockableStreamIB::Create(void* data, DWORD size, DWORD usage, DWORD fvf) {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT LockableStreamIB::Destroy() {
		Reset();
		LoseDevice();
	...
	LW_RESULT LockableStreamIB::Lock(UINT offset, UINT size, void** data, DWORD flag) {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT LockableStreamIB::Unlock() {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT LockableStreamIB::Bind(UINT base_vert_index) {
		return _dev_obj->SetIndices(_buf, base_vert_index);
	}

	LW_RESULT LockableStreamIB::LoseDevice() {
		if (_buf) {
			IDirect3DIndexBufferX* ib = _buf;
			_buf = 0;
			_dev_obj->ReleaseIndexBuffer(ib);
	...
	LW_RESULT LockableStreamIB::ResetDevice() {
		LW_RESULT ret = LW_RET_FAILED;
	...
	// LockableStreamMgr
	LW_STD_IMPLEMENTATION(LockableStreamMgr)

	LockableStreamMgr::LockableStreamMgr(IResourceMgr* res_mgr)
		: _res_mgr(res_mgr) {
	}

	LockableStreamMgr::~LockableStreamMgr() {
		LoseDevice();
	}

	LW_RESULT LockableStreamMgr::RegisterVertexBuffer(LW_HANDLE* handle, void* data, DWORD size, DWORD usage,
														DWORD fvf) {
		LW_RESULT ret = LW_RET_FAILED;

		LockableStreamVB* s = LW_NEW(LockableStreamVB(_res_mgr->GetDeviceObject()));
	...
	LW_RESULT LockableStreamMgr::RegisterIndexBuffer(LW_HANDLE* handle, void* data, DWORD size, DWORD usage,
													   DWORD fvf) {
		LW_RESULT ret = LW_RET_FAILED;

		LockableStreamIB* s = LW_NEW(LockableStreamIB(_res_mgr->GetDeviceObject()));
	...
	LW_RESULT LockableStreamMgr::UnregisterVertexBuffer(LW_HANDLE handle) {
		LW_RESULT ret = LW_RET_FAILED;
		...
		if (ret == LW_RET_OK_1) {
			// ~LockableStreamVB chama LoseDevice(), agora idempotente
			LW_DELETE(s);
		}
	...
	LW_RESULT LockableStreamMgr::UnregisterIndexBuffer(LW_HANDLE handle) {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT LockableStreamMgr::BindVertexBuffer(LW_HANDLE handle, UINT channel, UINT offset_byte, UINT stride) {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT LockableStreamMgr::BindIndexBuffer(LW_HANDLE handle, UINT base_vert_index) {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT LockableStreamMgr::LoseDevice() {
		LW_RESULT ret = LW_RET_OK;

		_pool_vb.ForEach([&](DWORD handle, void* raw) -> bool {
			auto* vb = static_cast<LockableStreamVB*>(raw);
		...
		_pool_ib.ForEach([&](DWORD handle, void* raw) -> bool {
			auto* ib = static_cast<LockableStreamIB*>(raw);
	...
	LW_RESULT LockableStreamMgr::ResetDevice() {
		LW_RESULT ret = LW_RET_OK;
	...
	ILockableStreamVB* LockableStreamMgr::GetStreamVB(LW_HANDLE handle) {
		LockableStreamVB* s = 0;
	...
	__ret:
		return (ILockableStreamVB*)s;
	}

	ILockableStreamIB* LockableStreamMgr::GetStreamIB(LW_HANDLE handle) {
		LockableStreamIB* s = 0;
	...
	LW_RESULT lwSurfaceStream::CreateRenderTarget(UINT width, UINT height, D3DFORMAT format,
		...
		IDeviceObject* dev_obj = _ss_mgr->GetResourceMgr()->GetDeviceObject();
	...
	lwSurfaceStreamMgr::lwSurfaceStreamMgr(IResourceMgr* res_mgr)
		: _res_mgr(res_mgr) {
	}
	...
	LW_RESULT lwVertexBuffer::Release() {
		...
		{
			DeviceObject* devobj = reinterpret_cast<DeviceObject*>(_dev_obj);
	...
	lwVertexBuffer::lwVertexBuffer(IDeviceObject* dev_obj)
	...
	lwIndexBuffer::lwIndexBuffer(IDeviceObject* dev_obj)
		: _dev_obj(dev_obj), _buf(0), _next(0), _prev(0) {
		_dlock_pos = 0;
		memset(&_buf_info, 0, sizeof(_buf_info));
	}
