...
namespace Corsairs::Engine::Render {
	...
	public:
		...
		void SetColorKeyType(ColorKeyType type, const lwColorValue4b* c) {
			_colorkey_type = type;
			if (c) {
				_colorkey = *c;
			}
	...
	public:
		...
		ILockableStreamVB* GetLockableStreamVB();
		ILockableStreamIB* GetLockableStreamIB();
	...
	public:
		lwMtlTexAgent(IResourceMgr* mgr);
		~lwMtlTexAgent();
	...
	public:
		lwMeshAgent(IResourceMgr* res_mgr);
		~lwMeshAgent();
	...
	class lwResBufMgr : public lwIResBufMgr {
		...
		typedef SlotMapVoidPtr10240 lwPoolSysMemTex;
	...
	private:
		...
		CriticalSection _lock_sysmemtex;
	...
	public:
		lwResBufMgr(IResourceMgr* res_mgr);
		~lwResBufMgr();
	...
	class ThreadPoolMgr : public IThreadPoolMgr {
		LW_STD_DECLARATION()
	...
	private:
		IThreadPool* _pool_seq[static_cast<std::size_t>(ThreadPoolType::THREAD_POOL_SIZE)];
		std::mutex _cs_seq[CRITICALSECTION_SEQ_SIZE];
	...
	public:
		...
		IThreadPool* GetThreadPool(ThreadPoolType type) {
			return _pool_seq[static_cast<std::size_t>(type)];
		}
	...
	class ResourceMgr : public IResourceMgr {
		typedef SlotMapVoidPtr10240 lwPoolMesh;
		typedef SlotMapVoidPtr40960 lwPoolTex;
		typedef SlotMapVoidPtr1024 lwPoolMeshRender;
		typedef SlotMapVoidPtr1024 lwPoolTexRender;
		typedef SlotMapVoidPtr1024 lwPoolAnimCtrl;
	...
	private:
		...
		IStaticStreamMgr* _static_stream_mgr;
		IDynamicStreamMgr* _dynamic_stream_mgr;
		ILockableStreamMgr* _lockable_stream_mgr;
		lwISurfaceStreamMgr* _surface_stream_mgr;
		IShaderMgr* _shader_mgr;
		lwIResBufMgr* _resbuf_mgr;
		IThreadPoolMgr* _thread_pool_mgr;

		lwPoolMesh _pool_mesh;
		lwPoolTex _pool_tex;
		lwPoolAnimCtrl _pool_animctrl;

		SlotMapVoidPtr10240 _pool_model;
	...
	public:
		ResourceMgr(ISysGraphics* sys);
		~ResourceMgr();

		ISysGraphics* GetSysGraphics() {
		...
		IShaderMgr* GetShaderMgr() {
			return _shader_mgr;
		}

		IStaticStreamMgr* GetStaticStreamMgr() {
			return _static_stream_mgr;
		}

		IDynamicStreamMgr* GetDynamicStreamMgr() {
			return _dynamic_stream_mgr;
		}

		ILockableStreamMgr* GetLockableStreamMgr() {
		...
		IThreadPoolMgr* GetThreadPoolMgr() {
		...
		LW_RESULT CreateHelperObject(IHelperObject** ret_obj);
		...
		LW_RESULT CreateStaticStreamMgr(IStaticStreamMgr** mgr);
		LW_RESULT CreateDynamicStreamMgr(IDynamicStreamMgr** mgr);
