...
#define OP_MATRIX_GLOBAL		2
	...
	inline int lwFloatZero(float f, float e) {
		return fabsf(f) < e;
	}

	inline int lwFloatEqual(float f1, float f2, float e) {
		return lwFloatZero(f1 - f2, e);
	}

	inline int lwFloatZero(float f) {
		return lwFloatZero(f, epsilon);
	}

	inline int lwFloatEqual(float f1, float f2) {
		return lwFloatZero(f1 - f2, epsilon);
	}

	inline int lwFloatZero(double f, double e) {
		return fabs(f) < e;
	}

	inline int lwFloatEqual(double f1, double f2, double e) {
		return lwFloatZero(f1 - f2, e);
	}
...
#define LWRECT_DECL
	...
	lwVector3* lwVector3RotateAxisAngle(lwVector3* out, const lwVector3* v, const lwVector3* axis_v0,
										const lwVector3* axis_v1, float angle);
	...
	lwMatrix44* lwMatrix44Slerp(const lwVector3* p0, const lwVector3* p1, const lwVector3* s0, const lwVector3* s1,
								const lwQuaternion* r0, const lwQuaternion* r1, float t, lwMatrix44* mat);
	...
	int lwCheckVectorInTriangle2D2(int* flag, const lwVector2* vp, const lwVector2* v1, const lwVector2* v2,
								   const lwVector2* v3);

	// check vector in triangle
	int lwCheckVectorInTriangle3D(const lwVector3* v, const lwVector3* v0, const lwVector3* v1, const lwVector3* v2,
								  const lwVector3* n);
	...
	int lwCheckVectorInTriangle3D2(int* flag, const lwVector3* v, const lwVector3* v0, const lwVector3* v1,
								   const lwVector3* v2, const lwVector3* n);
	...
	int lwIntersectLineLine2D(lwVector2* v, const lwVector2* u1, const lwVector2* u2, const lwVector2* v1,
							  const lwVector2* v2, int be_ray);
	int lwIntersectLineLine3D(lwVector3* v, const lwVector3* u1, const lwVector3* u2, const lwVector3* v1,
							  const lwVector3* v2, int be_ray);
	...
	LW_RESULT lwIntersectLineRect(lwVector2* o_v, int* o_n, const lwVector2* u1, const lwVector2* u2,
								  const lwVector2* rc_v1, const lwVector2* rc_v2, int is_segment);
	...
	// t[in]: triangle 
	int lwIntersectRayTriangle(lwVector3* v, float* d, const lwVector3* org, const lwVector3* ray,
	...
	inline int lwFloatRoundDec(float f) {
		return static_cast<int>(f);
	}

	inline int lwFloatRoundInc(float f) {
		return lwFloatRoundDec(f) + 1;
	}

	inline float lwFloatDecimal(float f) {
		return (f - (float)lwFloatRoundDec(f));
	}

	inline DWORD lwConvertColor3fToDWORD(const float* c) {
		return (0xff000000 | (unsigned char)(c[0] * 255) | ((unsigned char)(c[1] * 255) << 8) | ((unsigned char)(c[2] *
			255) << 16));
	}
	...
	lwVector3* WINAPI lwPlaneIntersectLine(lwVector3* p_out, const lwPlane* p, const lwVector3* v1,
										   const lwVector3* v2);
