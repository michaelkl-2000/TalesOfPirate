...
namespace GUI {
	...
	void CGuildBankMgr::_OnClickGoldTake(CGuiData* pSender, int x, int y, DWORD key) {
		g_stUIBox.ShowNumberBox(_EnterGoldTake, 2000000000 - (g_stUIBoat.GetHuman()->getGameAttr()->get(ATTR_GD)),
								"Enter Gold", false);
	}
...
#define BANK_TYPE 1
		...
		if (pkItemRecord->lID == 2520 || pkItemRecord->lID == 2521 || pkItemRecord->lID == 6341 || pkItemRecord->lID ==
			6343
			|| pkItemRecord->lID == 6347 || pkItemRecord->lID == 6359 || pkItemRecord->lID == 6370 || pkItemRecord->lID
			== 6371
		...
		if (pkItemCmd->GetItemInfo()->GetIsPile() && pkItemCmd->GetTotalNum() > 1) {
			/**/
			m_pkNumberBox =
				g_stUIBox.ShowNumberBox(_MoveItemsEvent, pkItemCmd->GetTotalNum(), GetLanguageString(441).c_str(),
										false);
	...
	bool CGuildBankMgr::PopFromBank(CGoodsGrid& rkDrag, CGoodsGrid& rkSelf, int nGridID, CCommandObj& rkItem) {
		...
		if (pkItemCmd->GetItemInfo()->GetIsPile() && pkItemCmd->GetTotalNum() > 1) {
			/**/
			m_pkNumberBox =
				g_stUIBox.ShowNumberBox(_MoveItemsEvent, pkItemCmd->GetTotalNum(), GetLanguageString(442).c_str(),
										false);
