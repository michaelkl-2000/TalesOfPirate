...
namespace Corsairs::Engine::Render {
	...
	class DeviceObject : public IDeviceObject {
		friend class lwVertexBuffer;
		friend class lwIndexBuffer;

	private:
		SysGraphics* _sys_graphics;
	...
	public:
		DeviceObject(SysGraphics* sys_graphics);
		~DeviceObject();
		...
		lwD3DCreateParam* GetD3DCreateParam() {
			return &_d3d_create_param;
		}

		D3DDISPLAYMODE* GetAdapterDisplayMode() {
			return &_display_mode;
		}

		D3DCAPSX* GetDeviceCaps() {
			return &_dev_caps;
		}

		lwBackBufferFormatsInfo* GetBackBufferFormatsCaps() {
			return &_bbf_caps;
		}

		LW_RESULT CreateVertexBuffer(UINT length, DWORD usage, DWORD fvf, D3DPOOL pool, IDirect3DVertexBufferX** vb,
									 HANDLE* handle);
		LW_RESULT CreateIndexBuffer(UINT length, DWORD usage, D3DFORMAT fmt, D3DPOOL pool, IDirect3DIndexBufferX** ib,
									HANDLE* handle);
		LW_RESULT CreateTexture(IDirect3DTextureX** out_tex, const lwTexDataInfo* info, DWORD level, DWORD usage,
								DWORD format, D3DPOOL pool);
		LW_RESULT CreateTexture(IDirect3DTextureX** out_tex, UINT width, UINT height, UINT level, DWORD usage,
								D3DFORMAT format, D3DPOOL pool);
		LW_RESULT CreateTextureFromFileInMemory(IDirect3DTextureX** out_tex, void* data, UINT data_size, UINT width,
												UINT height, UINT mip_level, DWORD usage, D3DFORMAT format,
												D3DPOOL pool, DWORD filter, DWORD mip_filter, D3DCOLOR colorkey,
		...
		LW_RESULT CreateRenderTarget(IDirect3DSurfaceX** o_surface, UINT width, UINT height, D3DFORMAT format,
									 D3DMULTISAMPLE_TYPE multi_sample, DWORD multi_sample_quality, BOOL lockable,
									 HANDLE* handle);
		LW_RESULT CreateDepthStencilSurface(IDirect3DSurfaceX** o_surface, UINT width, UINT height, D3DFORMAT format,
											D3DMULTISAMPLE_TYPE multi_sample, DWORD multi_sample_quality, BOOL discard,
											HANDLE* handle);
		LW_RESULT CreateCubeTexture(IDirect3DCubeTextureX** o_tex, UINT edge_length, UINT levels, DWORD usage,
									D3DFORMAT format, D3DPOOL pool, HANDLE* handle);
		LW_RESULT CreateOffscreenPlainSurface(IDirect3DSurfaceX** surface, UINT width, UINT height, D3DFORMAT format,
											  DWORD pool, HANDLE* handle);
		...
		LW_RESULT DrawIndexedPrimitive(D3DPRIMITIVETYPE pt_type, INT base_vert_index, UINT min_index, UINT vert_num,
									   UINT start_index, UINT count);
		LW_RESULT DrawPrimitiveUP(D3DPRIMITIVETYPE pt_type, UINT count, const void* data, UINT stride);
		LW_RESULT DrawIndexedPrimitiveUP(D3DPRIMITIVETYPE pt_type, UINT min_vert_index, UINT vert_num, UINT count,
										 const void* index_data, D3DFORMAT index_data_fmt, const void* vert_data,
										 UINT vert_stride);
		...
		LW_RESULT SetMaterial(lwMaterial* mtl) {
			return _dev->SetMaterial((D3DMATERIALX*)mtl);
		}
		...
		DWORD GetMarkVertexNum() const {
			return _mark_vertex_num;
		}

		DWORD GetMarkPolygonNum() const {
			return _mark_polygon_num;
		}

		lwWatchDevVideoMemInfo* GetWatchVideoMemInfo() {
			return &_watch_vm_info;
		}
