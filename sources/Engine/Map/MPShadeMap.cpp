...
bool CMPShadeMap::SetGridNum(int iNum) {
	...
	if (DWORD(_iVerNum * 2 + 8) > _vsConst)
		_useSoft = TRUE;
	...
	_pModel->CreateShadeModel(_iVerNum, _iFaceCount, _iGridCrossNum, _useSoft);
...
void CMPShadeMap::RenderVS() {
	...
	_pCEffectFile->_dev->SetRenderState(D3DRS_ZENABLE, FALSE);
	_pModel->_dev->SetRenderState(D3DRS_ZENABLE, TRUE);

	_pModel->_dev->SetRenderStateForced(D3DRS_SRCBLEND, _eSrcBlend);
	_pModel->_dev->SetRenderStateForced(D3DRS_DESTBLEND, _eDestBlend);

	_pModel->_dev->SetSamplerState(0, D3DSAMP_MAGFILTER, D3DTEXF_LINEAR);
	_pModel->_dev->SetSamplerState(0, D3DSAMP_MINFILTER, D3DTEXF_LINEAR);

	_pModel->_dev->SetVertexShader(CMPResManger::Instance().GetShadeVS());
	...
	for (int n = 0; n < _iVerNum; n++) {
		tv.x = _SShadeUV[n].x;
		tv.y = _SShadeUV[n].y;
		_pModel->_dev->SetVertexShaderConstantF(nIndex++, tv, 1);
...
void CMPShadeMap::RenderSoft() {
	...
	_pModel->Begin();
	_pCEffectFile->_dev->SetRenderState(D3DRS_TEXTUREFACTOR, _dwColor);
	_pCEffectFile->_dev->SetTextureStageStateForced(0, D3DTSS_COLORARG2, D3DTA_TFACTOR);
	_pCEffectFile->_dev->SetTextureStageStateForced(0, D3DTSS_ALPHAARG2, D3DTA_TFACTOR);
...
void CMPShadeCtrl::setFrameCount(int iCount) {
	if (_pShadeMap->_shadeType == SHADE_ANI)
...
void CMPShadeCtrl::setColor(D3DXCOLOR SColor) {
	if (_pShadeMap->_shadeType == SHADE_SINGLE)
...
bool CMPShadeCtrl::SaveToFile(char* pchName) {
	...
	if (!pFile)
		return false;
	fwrite(&_pShadeMap->_shadeType, sizeof(int), 1, pFile);
