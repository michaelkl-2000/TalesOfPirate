...
public:
	...
	void setSection(int nSectionNO) {
		_nSectionNO = nSectionNO;
	}
	...
	void setObjType(DWORD type) {
		_dwObjType = type;
	}

	DWORD getObjType() const {
		return _dwObjType;
	}
	...
	void SetMusicID(int nMusicID) {
		_nMusicID = nMusicID;
	}

	//lemon add@2004.10.28 for effect light
	void setChaID(int iId) {
		_iChaID = iId;
	}
...
public:
	...
	void SetCullingFlag(DWORD flag) {
		_dwCullingFlag = flag;
	}
