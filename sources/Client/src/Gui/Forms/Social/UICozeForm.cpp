...
CCharMsg::CCharMsg() {
	m_ecboShowChannels = CHANNEL_ALL | CHANNEL_SIGHT | CHANNEL_PRIVATE | CHANNEL_WORLD | CHANNEL_TRADE | CHANNEL_TEAM |
		CHANNEL_GUILD | CHANNEL_SYSTEM | CHANNEL_PUBLISH | CHANNEL_SIDE;
...
bool CCharMsg::ModifyShowChannel(eChannel eShowChannel, bool bAddOrRemove, bool bShowTips) {
	...
	if (bOrgShow != bAddOrRemove) {
		if (bAddOrRemove) {
			...
			if (bShowTips) {
				CCozeForm::GetInstance()->OnSystemMsg(GetChannelName(eShowChannel) + GetLanguageString(500));
			}
		}
		else {
			SetShowChannels(m_ecboShowChannels & ~eShowChannel);
			if (bShowTips) {
				CCozeForm::GetInstance()->OnSystemMsg(GetChannelName(eShowChannel) + GetLanguageString(501));
			}
		}
		return true;
	}
...
CCharMsg::sTextInfo CCharMsg::GetMsgInfo() {
	...
	sText.strWho = GetLanguageString(2);
	sText.strText = GetLanguageString(502);
	sText.strShowText = GetLanguageString(502);
	sText.bSendTo = false;
...
void CChannelSwitchForm::EventLostFocus(CGuiData* pSender) {
	CCozeForm* pCozeForm = CCozeForm::GetInstance();
	pCozeForm->m_chkChannelSwitch->SetIsChecked(false);
	pCozeForm->m_chkChannelSwitch->MouseRun(pCozeForm->m_chkChannelSwitch->GetX() - 1,
...
void CChannelSwitchForm::EventPrivateCheckChange(CGuiData* pSender) {
	CCozeForm* pCozeForm = CCozeForm::GetInstance();
	pCozeForm->m_cMainMsg.ModifyShowChannel(CCharMsg::CHANNEL_PRIVATE,
...
void CChannelSwitchForm::EventSightCheckChange(CGuiData* pSender) {
	CCozeForm* pCozeForm = CCozeForm::GetInstance();
	pCozeForm->m_cMainMsg.ModifyShowChannel(CCharMsg::CHANNEL_SIGHT,
...
void CChannelSwitchForm::EventSystemCheckChange(CGuiData* pSender) {
	...
	if (bCheck) {
		pCozeForm->OnSystemMsg(GetLanguageString(503));
	}
	else {
		pCozeForm->OnSystemMsg(GetLanguageString(504));
	}
	pCozeForm->ResetPages();
}
...
void CChannelSwitchForm::EventTeamCheckChange(CGuiData* pSender) {
	CCozeForm* pCozeForm = CCozeForm::GetInstance();
	pCozeForm->m_cMainMsg.ModifyShowChannel(CCharMsg::CHANNEL_TEAM,
...
void CChannelSwitchForm::EventGuildCheckChange(CGuiData* pSender) {
	CCozeForm* pCozeForm = CCozeForm::GetInstance();
	pCozeForm->m_cMainMsg.ModifyShowChannel(CCharMsg::CHANNEL_GUILD,
...
void CChannelSwitchForm::EventWorldCheckChange(CGuiData* pSender) {
	CCozeForm* pCozeForm = CCozeForm::GetInstance();
	pCozeForm->m_cMainMsg.ModifyShowChannel(CCharMsg::CHANNEL_WORLD,
...
void CChannelSwitchForm::EventTradeCheckChange(CGuiData* pSender) {
	CCozeForm* pCozeForm = CCozeForm::GetInstance();
	pCozeForm->m_cMainMsg.ModifyShowChannel(CCharMsg::CHANNEL_TRADE,
...
void CCozeForm::OnPrivateMsg(string strFromName, string strToName, string strMsg, DWORD dwColour) {
	...
	if (strHumanName == strFromName) {
		...
		if (strMsg == "{x}*") {
			OnSystemMsg(GetLanguageString(505) + strFromName + GetLanguageString(506));
		}
		else {
...
void CCozeForm::OnPublishMsg(string strName, string strMsg) {
	string str;
	str = GetLanguageString(507) + string(strMsg);
...
void CCozeForm::SendMsg() {
		...
		if (p != std::string::npos) {
			...
			if (!strContent.empty()) {
				...
				if (!strPersonName.empty() && strPersonName.length() <= 37) {
					...
					strCurCmd = CCharMsg::GetChannelCommand(CCharMsg::GetChannelIndex(CCharMsg::CHANNEL_PRIVATE)) +
						strPersonName + " ";
	...
	case CCharMsg::CHANNEL_SIGHT: {
		...
		if (preStr == strMsg && GetTickCount() - preTime < 1000) {
			OnSystemMsg(GetLanguageString(508));
		}
		else {
	...
	case CCharMsg::CHANNEL_WORLD: {
		...
		if (!pChar || pChar->getGameAttr()->get(ATTR_LV) < 10) {
			OnSystemMsg(GetLanguageString(509));
		}
		else // modify by Philip.Wu  2006-06-09   10
		{
			if (pChar->getGMLv() == 99 || GetTickCount() - g_pGameApp->GetLoginTime() > 60000) {
				...
				OnSystemMsg(GetLanguageString(510));
			}
		}
		break;
	}
	case CCharMsg::CHANNEL_TRADE: {
		CCharacter* pChar = g_stUIBoat.GetHuman();
		if (!pChar || pChar->getGameAttr()->get(ATTR_LV) < 10) {
			OnSystemMsg(GetLanguageString(511));
		}
		else // modify by Philip.Wu  2006-06-09   10
		{
			if (pChar->getGMLv() == 99 || GetTickCount() - g_pGameApp->GetLoginTime() > 60000) {
				...
				OnSystemMsg(GetLanguageString(512));
			}
		}
		break;
	}
	case CCharMsg::CHANNEL_TEAM: {
		...
		if (!strMsg.empty()) {
			...
			if (!pTeam || pTeam->GetCount() == 0) {
				OnSystemMsg(GetLanguageString(513));
			}
			else {
	...
	default: {
		OnSystemMsg(GetLanguageString(514));
		return;
	}
	}
...
void CCozeForm::EventSendChannelChange(CGuiData* pSender) {
	...
	pThis->m_edtMsg->
		   SetCaption(CCharMsg::GetChannelCommand(CCharMsg::GetChannelIndex(pThis->m_eCurSelChannel)).c_str());
}
...
void CCozeForm::EventCallingCardSwitchClick(CGuiData* pSender, int x, int y, DWORD key) {
	...
	CItemRow* pRow = pThis->m_lstCallingCard->Add(GetLanguageString(515).c_str());
