		...
		if (_pTarget) {
			...
			if (GetDistance(_pMain->GetCurX(), _pMain->GetCurY(), _nAttackX, _nAttackY) > _nTotalDis) {
				...
				if (!g_cFindPath.GetPathFindingState()
					/*g_cFindPath.Find( _pMain->GetScene(), _pMain, _pMain->GetCurX(), _pMain->GetCurY(), _nAttackX, _nAttackY, IsWalkLine )*/) {
					_eStyle = eNone;
					return;
				}
			}
	...
	case eFollow: {
		if (!_pMain->IsValid() || !_pMain->IsEnabled() || !_pTarget->IsValid() || !_pTarget->IsPlayer()) {
			...
			if (CGameApp::GetCurTick() > time || (_pMain->GetIsArrive() && _pMain->GetActor()->IsEmpty())) {
				...
				const int dis = 200;
				if (GetDistance(_pMain->GetServerX(), _pMain->GetServerY(), _pTarget->GetServerX(),
								_pTarget->GetServerY()) > dis) {
					int x, y;
					GetDistancePos(_pTarget->GetServerX(), _pTarget->GetServerY(), _pMain->GetServerX(),
								   _pMain->GetServerY(), dis - 100, x, y);
