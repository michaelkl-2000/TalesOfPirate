...
namespace Corsairs::Engine::Render {
	...
	LW_RESULT lwPhysique::LoadBone(std::string_view file) {
		...
		res.obj_id = 0;
		res.res_type = ResourceFileType::RES_FILE_TYPE_GENERIC;
	...
	LW_RESULT lwPhysique::LoadPrimitive(DWORD part_id, std::string_view file) {
		...
		const std::string path = std::format("{}{}", path_info->GetPath(PathInfoType::PATH_TYPE_MODEL_CHARACTER), file);
		...
		lwResFileMesh rfm;
		rfm.obj_id = 0;
		rfm.res_type = ResourceFileType::RES_FILE_TYPE_GEOMETRY;
