...
void NetPCTeam(stNetPCTeam& pcteam) {
	...
	ToLogService("players", "Kind:[{}], LeaderID:{}\n", pcteam.kind, pcteam.cha_dbid[0]);
	...
	case TEAM_MSG_ADD: // :
	{
		CTeam* pTeam = pTeamMgr->Find(enumTeamGroup);
		if (pTeam) {
			pTeam->Add(pcteam.cha_dbid[pcteam.count - 1], pcteam.cha_name[pcteam.count - 1],
	...
	break;
	default: ToLogService("network", "msgNetPCTeam unknown kind:{}", pcteam.kind);
	}
}
...
void NetFrndAdd(unsigned long cha_id, const char* cha_name, const char* motto, unsigned short icon_id,
				const char* grp) {
	//grp-
	//  dev-only  test_frnd
...
void NetGMStart(stNetFrndStart arrCha[], unsigned short arrnum) {
	for (int i = 0; i < arrnum; i++) {
		CMember* pMember = g_stUIChat.GetTeamMgr()->Find(enumTeamGM)->Add(
			arrCha[i].lChaid, arrCha[i].szChaname.c_str(), arrCha[i].szMotto.c_str(), arrCha[i].sIconID);
		if (pMember) {
...
void NetFrndStart(stNetFrndStart& self, stNetFrndStart arrCha[], unsigned short arrnum) {
	...
	for (int i = 0; i < arrnum; i++) {
		...
		CMember* pMember = g_stUIChat.GetTeamMgr()->Find(enumTeamFrnd)->Add(
			arrCha[i].lChaid, arrCha[i].szChaname.c_str(), arrCha[i].szMotto.c_str(), arrCha[i].sIconID);
...
void NetFrndRefreshInfo(unsigned long cha_id, const char* motto, unsigned short icon, unsigned short degree,
...
void NetMasterAdd(unsigned long cha_id, const char* cha_name, const char* motto, unsigned short icon_id,
				  const char* grp) {
	//  dev-only  test_frnd
...
void NetMasterStart(stNetFrndStart& self, stNetFrndStart arrCha[], unsigned short arrnum) {
	...
	for (int i = 0; i < arrnum; i++) {
		//  dev-only  test_frnd
		CMember* pMember = g_stUIChat.GetTeamMgr()->Find(enumTeamMaster)->Add(
			arrCha[i].lChaid, arrCha[i].szChaname.c_str(), arrCha[i].szMotto.c_str(), arrCha[i].sIconID);
		pMember->SetOnline(arrCha[i].cStatus != 0);
	}
	g_stUIChat._UpdateSelfInfo();
...
void NetMasterRefreshInfo(unsigned long cha_id, const char* motto, unsigned short icon, unsigned short degree,
						  const char* job, const char* guildname) {
	//  dev-only  test_frnd
	CMember* pMember = g_stUIChat.GetTeamMgr()->Find(enumTeamMaster)->Find(cha_id);
	if (pMember) {
...
void NetPrenticeAdd(unsigned long cha_id, const char* cha_name, const char* motto, unsigned short icon_id,
					const char* grp) {
	//  dev-only  test_frnd
...
void NetPrenticeStart(stNetFrndStart& self, stNetFrndStart arrCha[], unsigned short arrnum) {
	...
	for (int i = 0; i < arrnum; i++) {
		//  dev-only  test_frnd
		CMember* pMember = g_stUIChat.GetTeamMgr()->Find(enumTeamPrentice)->Add(
			arrCha[i].lChaid, arrCha[i].szChaname.c_str(), arrCha[i].szMotto.c_str(), arrCha[i].sIconID);
		pMember->SetOnline(arrCha[i].cStatus != 0);
	}
	g_stUIChat._UpdateSelfInfo();
...
void NetPrenticeRefreshInfo(unsigned long cha_id, const char* motto, unsigned short icon, unsigned short degree,
							const char* job, const char* guildname) {
	//  dev-only  test_frnd
	CMember* pMember = g_stUIChat.GetTeamMgr()->Find(enumTeamPrentice)->Find(cha_id);
	if (pMember) {
