...
inline char* CInputBox::RefreshText() {
	...
	const std::string utf8 = Corsairs::Util::WideToUtf8(std::wstring_view(wbuf, static_cast<std::size_t>(wlen)));
...
inline char* CInputBox::SetText(const char* pszText) {
	if (!pszText) pszText = "";
	const std::wstring wide = Corsairs::Util::Utf8ToWide(pszText);
...
inline void CInputBox::ReplaceSel(const char* pszRplText, BOOL bCanUndo) {
	...
	const std::wstring wide = Corsairs::Util::Utf8ToWide(pszRplText);
