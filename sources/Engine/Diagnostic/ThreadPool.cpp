...
namespace Corsairs::Engine::Render {
	LW_STD_IMPLEMENTATION(ThreadPool)

	void ThreadPool::_ThreadProc(std::size_t worker_idx) {
		_thread_states[worker_idx].store(THREADTASKSTATE_WAITING, std::memory_order_release);

		while (true) {
			ThreadTaskInfo task_info{nullptr, nullptr};
	...
	LW_RESULT ThreadPool::Create(DWORD thread_seq_size, DWORD task_seq_size, DWORD suspend_flag) {
		if (!_threads.empty()) {
		...
		try {
			for (std::size_t i = 0; i < thread_seq_size; ++i) {
				_threads.emplace_back(&ThreadPool::_ThreadProc, this, i);
			}
		}
	...
	LW_RESULT ThreadPool::RegisterTask(ThreadProc proc, void* param) {
		if (proc == nullptr) {
			return LW_RET_FAILED;
		}

		{
			...
			_queue_task.push_back(ThreadTaskInfo{proc, param});
		}
		_cv.notify_one();
	...
	LW_RESULT ThreadPool::RemoveTask(ThreadProc proc, void* param) {
		std::lock_guard lock(_mu);
		auto it = std::find_if(_queue_task.begin(), _queue_task.end(),
							   [&](const ThreadTaskInfo& t) {
	...
	LW_RESULT ThreadPool::FindTask(ThreadProc proc, void* param) {
		std::lock_guard lock(_mu);
		auto it = std::find_if(_queue_task.begin(), _queue_task.end(),
							   [&](const ThreadTaskInfo& t) {
								   return t.proc == proc && t.param == param;
	...
	LW_RESULT ThreadPool::SuspendThread() {
		// Legacy stub — оригинал возвращал -1L. Сохраняем поведение для
		// совместимости с IThreadPool. Реальный suspend делается через
		// SetPoolEvent(TRUE).
		return -1L;
	}

	LW_RESULT ThreadPool::ResumeThread() {
		return -1L;
	}

	LW_RESULT ThreadPool::SetPriority(int priority) {
	...
	LW_RESULT ThreadPool::SetPoolEvent(BOOL lock_flag) {
	...
	DWORD ThreadPool::GetCurrentRunningTaskNum() const {
		DWORD n = 0;
	...
	DWORD ThreadPool::GetCurrentIdleThreadNum() const {
