...
public:
	...
	void PlayPose(DWORD pose, DWORD type = PLAY_ONCE, int time = -1, int fps = 32, DWORD blend_flag = 1,
				  DWORD blend_src_num = 5, bool IsGlitched = false); // LoopSpeed,Pose
	...
	CSceneItem* GetLinkItem(DWORD link_id) {
		return link_id < 0 || link_id >= LINK_ID_NUM ? 0 : _LinkItem[link_id];
	}

	void SetModelType(DWORD type) {
		_ModelType = type;
	}

	DWORD GetModelType() const {
		return _ModelType;
	}

	void SetUIScaleDis(float dis) {
		_UIScaleDis = dis;
	}

	void SetUIYaw(int angle) {
		_UIYaw = angle;
	}

	void SetUIPitch(int angle) {
		_UIPitch = angle;
	}
	...
	DWORD GetPartID(DWORD id) const {
		return _PartID[id];
	}

	DWORD GetShipType() const {
		return _ShipType;
	}

	void SetShipType(DWORD type) {
		_ShipType = type;
	}
	...
	void SetEmiColorFLag(BOOL flag) {
		_EmiColor = flag;
	}

	BOOL GetEmiColorFlag() {
		return _EmiColor;
	}
