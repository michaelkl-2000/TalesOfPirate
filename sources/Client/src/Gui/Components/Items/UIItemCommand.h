...
using CStoneInfo = Corsairs::Common::Item::CStoneInfo;
using CItemRefineInfo = Corsairs::Common::Item::CItemRefineInfo;
using CItemRefineEffectInfo = Corsairs::Common::Item::CItemRefineEffectInfo;
struct SGameAttr;
...
namespace GUI {
	...
	public:
		...
		virtual bool GetCanDrag() {
			return _canDrag;
		};

		virtual void SetCanDrag(bool drag) {
			_canDrag = drag;
		};
		...
		bool IsLocked() const {
			return static_cast<bool>(_ItemData.dwDBID);
		}

		CItemRecord* GetItemInfo() {
			return _pItem;
		}
