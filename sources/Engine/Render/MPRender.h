...
public:
	...
	void EnableClearTarget(BOOL bEnable = TRUE) {
		_bClearTarget = bEnable;
	}

	void EnableClearZBuffer(BOOL bEnable = TRUE) {
		_bClearZBuffer = bEnable;
	}

	void EnableClearStencil(BOOL bEnable = TRUE) {
		_bClearStencil = bEnable;
	}
	...
	// View Routines
	void reset_Camera_Project_VIEW_WORLD(D3DXMATRIX* mat, const float fov_y, const float aspect, const float zn,
										 const float zf);
	void reset_Camera_Project_VIEW_UI(D3DXMATRIX* mat, const float fov_y, const float aspect, const float zn,
									  const float zf);
	...
	float GetWorldViewFOV() {
		return _fWorldViewFOV;
	}

	float GetWorldViewAspect() {
		return _fAspect;
	}
	...
	int GetWorldViewX() {
		return _nWorldViewStartX;
	}

	int GetWorldViewY() {
		return _nWorldViewStartY;
	}

	int GetWorldViewWidth() {
		return _nWorldViewWidth;
	}

	int GetWorldViewHeight() {
		return _nWorldViewHeight;
	}

	D3DXMATRIX& GetWorldProjMatrix() {
		return _matProjWorld;
	}

	D3DXMATRIX& GetWorldViewMatrix() {
		return _matViewWorld;
	}
	...
	D3DXMATRIX& GetUIViewMatrix() {
		return _matUIView;
	}

	D3DXMATRIX& GetUIProjMatrix() {
		return _matUIProj;
	}
	...
	void EnableCaptureAVI(BOOL bEnable) {
		_bEnableCaptureAVI = bEnable;
	}

	BOOL IsEnableCaptureAVI() {
		return _bEnableCaptureAVI;
	}

	void CaptureScreen() {
		_bCaptureScreen = TRUE;
	}
	...
	D3DFORMAT GetTexSetFormat(DWORD type) {
		return _TexSetFmt[type];
	}
...
protected:
	...
	D3DXMATRIXA16 _matViewWorld; // View	Matrix
	D3DXMATRIXA16 _matUIView; // View Matrix
	D3DXMATRIXA16 _matUIProj; // Proj Matrix
	D3DXMATRIXA16 _matViewProj; // = Proj * View, GetRay		
	D3DXMATRIXA16 _mat3DUIView;
	D3DXMATRIXA16 _mat3DUIProj;

	int _nWorldViewStartX; // 
	int _nWorldViewStartY;
	int _nWorldViewWidth; // 
	int _nWorldViewHeight;
	DWORD _dwBackgroundColor; // 
	DWORD _dwClearFlag;
	BOOL _bClearTarget;
	BOOL _bClearStencil;
	BOOL _bClearZBuffer;
	...
	float _fAspect;
	int _nCurViewType; // SetCurrentView
	float _fNearClip;
	float _fFarClip;
...
inline HRESULT MPRender::SetStreamSource(UINT stream_num, IDirect3DVertexBufferX* stream_data, UINT offset_byte,
										 UINT stride) {
...
inline HRESULT MPRender::DrawIndexedPrimitive(D3DPRIMITIVETYPE pt_type, INT base_vert_index, UINT min_index,
