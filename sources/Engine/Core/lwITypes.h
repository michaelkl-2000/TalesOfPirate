...
// Общий std::format-formatter для любого `enum class` со scope: форматирует
// как underlying-тип (по умолчанию std::uint32_t). Покрывает все enum class
// проекта одной partial-специализацией, без boilerplate на каждый enum.
// Full-специализации (как `std::formatter<LogLevel>`) приоритетнее этой
// partial и продолжают работать.
template <typename E>
    requires std::is_scoped_enum_v<E>
struct std::formatter<E, char> : std::formatter<std::underlying_type_t<E>, char> {
...
#define LW_RENDER_CTRL_PROC_NUM         512
	...
	// PrimitiveLW_MAX_OBJ_DUMMY_NUMDummyDummy
	// LW_MAX_OBJ_DUMMY_INV_NUMmaxdummydummyid
	// dummy_0 - dummy_3LW_MAX_OBJ_DUMMY_INV_NUM
	//               dummy_4 - dummy_7Inverse Matrixid
...
#define MAX_KEY_FRAME_NUM           8
	...
	enum {
		...
		GEOMOBJ_TYPE_CHECK_BB = 1, //
		GEOMOBJ_TYPE_CHECK_BB2 = 2, //( 0, 1, 0 )
	};
	...
	// *.lmo, *.lgo, *.lab, *.lam, *.lat
	enum class ResourceFileType : std::uint32_t {
	...
	enum class PathInfoType : std::uint32_t {
	...
	enum class ColorKeyType : std::uint32_t {
...
#define LW_COLOR_A( c ) (BYTE)( ((c) >> 24) & 0xff )
	...
	struct lwResFile {
		...
		ResourceFileType res_type;
		DWORD obj_id;

		lwResFile()
			: obj_id(0), res_type(ResourceFileType::RES_FILE_TYPE_INVALID) {
			file_name[0] = '\0';
		}
	};
	...
	struct lwResFileAnimData {
		...
		lwResFileAnimData()
			: obj_id(0), res_type(ResourceFileType::RES_FILE_TYPE_INVALID), anim_type(ANIM_CTRL_TYPE_INVALID) {
			file_name[0] = '\0';
		}
	...
	enum class AnimCtrlAgentSetType : std::uint32_t {
		ACA_GLOBAL_SET,
		ACA_SUBSET_SET,
	};
	...
	// NOTE: plain enum (не class) — namespace_PRIMITIVE и др. используются как
	// значения DWORD-полей в .lgo/.lmo-сериализации (lwExpObj.h::_type).
	// Конвертация в enum class требует static_cast в каждом колсайте; не делаем
	// из-за риска поломать disk layout/serialization.
	...
	enum class TreeNodeProcType : std::uint32_t {
	...
	// TODO(Ф1): TreeNodeProcRetType — оставлен plain enum, т.к. возвращается из
	// callback-typedef `lwTreeNodeEnumProc` (DWORD), используется в десятках
	// callback'ов (`__tree_proc_*`/lambda'ах) и присваивается локальным DWORD ret.
	// Перевод в enum class требует синхронной правки typedef'а + всех callback'ов
	// + локальных переменных. Сделать в Ф6 (нейминг полей) или в отдельной enum-фазе.
	...
	enum class ModelObjectLoadType : std::uint32_t {
	...
	enum class SurfaceStreamType : std::uint32_t {
	...
	enum class CriticalSectionType : std::uint32_t {
