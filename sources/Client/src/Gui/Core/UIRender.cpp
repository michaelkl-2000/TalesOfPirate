...
void CUIPanel::Draw(IDirect3DTextureX* pSrcTexture,
...
bool CDrag::MouseRun(int x, int y, DWORD key) {
	...
	if (key & Mouse_LUp) {
		if (_IsUseGrid) {
			_nDragX = (_pParent->GetX() + GetDragX() + _dwGridWidth / 2) / _dwGridWidth * _dwGridWidth - _pParent->
				GetX();
			_nDragY = (_pParent->GetY() + GetDragY() + _dwGridHeight / 2) / _dwGridHeight * _dwGridHeight - _pParent->
				GetY();
		}
...
bool CFormMgr::Init(HWND hWnd) {
	if (!_bInit) {
		...
		// накапливают UTF-8 через Corsairs::Util::Encoding::AppendAnsiByteAsUtf8, IME — через
