...
public:
	...
	S_BVECTOR<D3DXVECTOR3>& GetResultPath() {
		return _vecPathPoint;
	}
	...
	bool Find(CGameScene* pScene, CCharacter* pCha, int nSelfX, int nSelfY, int nTargetX, int nTargetY,
			  bool& IsWalkLine);

	bool GetPathFindingState() {
		return m_bFindPath;
	}

	bool LongPathFinding() {
		return m_bLongPathFinding;
	}
...
protected:
	...
	BOOL FindPath(CGameScene* pScene, CCharacter* pCha, int nSelfX, int nSelfY, int nTargetX, int nTargetY,
				  bool& IsWalkLine);
