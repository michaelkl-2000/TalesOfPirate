...
int CCharacter::LoadCha(const LoadChaInfo* info) {
	...
	if (info->bone[0] != 0) {
		if (FAILED(LoadBone( info->bone ))) {
			g_logManager.InternalLog(LogLevel::Error, "errors", GetLanguageString(16));
			return FALSE;
		}
	}
...
CEffectObj* CCharacter::SkyEffect(int nEffectID, int nBeginDummy, int nItemDummy, int nSpeed, D3DXVECTOR3* pTarget,
								  int nTargetChaID, CSkillRecord* pSkill) {
	if (nEffectID <= 0) return NULL;
...
xShipInfo* CCharacter::ConvertPartTo8DWORD(stNetChangeChaPart& stPart, DWORD* dwBuf) {
	...
	if (!pInfo) {
		g_logManager.InternalLog(LogLevel::Debug, "common",
								 SafeVFormat(GetLanguageString(20), static_cast<int>(stPart.sBoatID)));
		return NULL;
	}
	...
	if (pData == NULL) {
		g_logManager.InternalLog(LogLevel::Debug, "common",
								 SafeVFormat(GetLanguageString(21), static_cast<int>(pInfo->sBody)));
		return NULL;
	}
	...
	if (pInfo->byIsUpdate) {
		...
		if (pData == NULL) {
			g_logManager.InternalLog(LogLevel::Debug, "common",
									 SafeVFormat(GetLanguageString(22), static_cast<int>(stPart.sHeader)));
			return NULL;
		}
		...
		if (pData == NULL) {
			g_logManager.InternalLog(LogLevel::Debug, "common",
									 SafeVFormat(GetLanguageString(23), static_cast<int>(stPart.sEngine)));
			return NULL;
		}
...
bool CCharacter::UpdataItem(int nItem, DWORD nLink) {
	...
	case enumEQUIP_CLOAK: {
		//edited to allow cloak glowing by level from iteminfo @mothannakh
	...
	case enumEQUIP_SHOES:
		...
		if (CCharacterModel::ChangePart(nLink, nItem) == 0) {
			g_logManager.InternalLog(LogLevel::Debug, "common",
									 std::format("msgChangePart Error, Item[{}], Link[{}]\n", nItem, nLink));
			return false;
		}
		return true;
		...
		if (nItem > 0 && nItem != enumEQUIP_BOTH_HAND) {
			...
			if (!_pHandItem[nLink] || _pHandItem[nLink]->GetItemInfo()->lID != nItem) {
				...
				if (item) {
					...
					g_logManager.InternalLog(LogLevel::Debug, "common",
											 std::format("msgChangePart AddSceneItem Error, Item[{}], Link[{}]\n",
														 nItem, nLink));
					return false;
				}
			}
...
void CCharacter::SynchroSkillState(stSkillState* pState, int nCount) {
	...
	if (set.IsTrue(static_cast<unsigned>(eChaState::enumChaStateMove)) != _ChaState.IsTrue(
		static_cast<unsigned>(eChaState::enumChaStateMove))) {
	...
	if (set.IsTrue(static_cast<unsigned>(eChaState::enumChaStateNoDizzy)) != _ChaState.IsTrue(
		static_cast<unsigned>(eChaState::enumChaStateNoDizzy))) {
		if (_pDefaultChaInfo->IsCyclone()) {
