...
CListItems::CListItems()
	: _pSelect(new CSelectItem()), _dwCount(0), _nItemLeft(0), _nItemTop(0)
	  , _dwFirstShowRow(0), _pList(NULL), _dwLastShowRow(0), _nShowCount(1000), _nMaxRow(0), _IsMouseFollow(false),
	  _pItemPic(NULL)
...
void CListItems::Render() {
	if (_pSelect->GetIsEnabled() && _pSelect->GetIsSelect() && _pSelect->GetIndex() >= (int)_dwFirstShowRow && _pSelect
		->GetIndex() < (int)_dwLastShowRow) {
...
void CList::Refresh() {
	...
	_pItems->SetRect(GetX() + _nLeftMargin, GetY() + _nTopMargin, GetX2() - _pScroll->GetWidth() - _nRightMargin,
