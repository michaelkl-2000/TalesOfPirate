...
public:
	...
	void setAttachedCharacterID(DWORD id) {
		_nCharacterID = id;
	}

	DWORD getAttachedCharacterID() const {
...
public:
	...
	void SetItemInfo(int id) {
		_pItemInfo = Corsairs::Common::Item::GetItemRecordInfo(id);
	}

	Corsairs::Common::Item::CItemRecord* GetItemInfo() {
		return _pItemInfo;
	}
...
protected:
	bool _IsSystem; // 
	bool _IsShowName;
	Corsairs::Common::Item::CItemRecord* _pItemInfo;
	int _nDrapID;
	CEvent* _pEvent;
