...
namespace Corsairs::Common::Inventory {
		...
		for (int i = 0; i < enumKBITEM_TYPE_NUM; i++) {
			...
			for (int j = 0; j < pKitbag->GetCapacity(); j++) {
				...
				sprintf(szData, "%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
						...
						pGridCont->sEndure[0], pGridCont->sEndure[1], pGridCont->sEnergy[0], pGridCont->sEnergy[1],
						pGridCont->chForgeLv, pGridCont->dwDBID, pGridCont->sNeedLv,
				...
				lnCheckSum += (pGridCont->sID + pGridCont->sNum + pGridCont->sEndure[0] + pGridCont->sEndure[1] +
					pGridCont->sEnergy[0] + pGridCont->sEnergy[1] + pGridCont->chForgeLv + pGridCont->dwDBID + pGridCont
					->sNeedLv + pGridCont->bIsLock + pGridCont->bItemTradable);
	...
	bool String2KitbagData(CKitbag* pKitbag, std::string& strData) {
		...
		const short csSubNum = 8 + Item::enumITEMDBP_MAXNUM + defITEM_INSTANCE_ATTR_NUM_VER110 * 2 + 1 + 1;
		std::string strSubList[csSubNum];
		std::string strVer[2];
		std::string strCap[2];
		bool bIsOldVer;
		int nSegNum;
		if (Corsairs::Util::ResolveTextLine(strData.c_str(), strCap, 2, '@') > 1) // ???????????
		{
			pKitbag->SetCapacity(Corsairs::Util::Str2Int(strCap[0]));
			bIsOldVer = Corsairs::Util::ResolveTextLine(strCap[1].c_str(), strVer, 2, '#') == 1 ? true : false;
		}
		else {
			bIsOldVer = Corsairs::Util::ResolveTextLine(strCap[0].c_str(), strVer, 2, '#') == 1 ? true : false;
		}
		...
		if (bIsOldVer) {
			...
			if (iVer == 113 || iVer == 114) {
				...
				buff[len] = '\0';
				nSegNum = Corsairs::Util::ResolveTextLine(buff, strList, csStrNum, ';');
			}
			else
				nSegNum = Corsairs::Util::ResolveTextLine(strVer[0].c_str(), strList, csStrNum, ';');
		}
		else {
			...
			if (iVer == 113 || iVer == 114) {
				...
				nSegNum = Corsairs::Util::ResolveTextLine(strVer[1].c_str(), strList, csStrNum, ';');
			}
			if (nSegNum < 2)
				return false;
		}
		...
		short sPageNum = Corsairs::Util::Str2Int(strList[sSegID++]);
		...
		for (int i = 0; i < sPageNum; i++) {
			sUseGridNum = Corsairs::Util::Str2Int(strList[sSegID++]);
			for (int j = 0; j < nItemNumPerPage; j++) {
				sTCount = 0;
				Corsairs::Util::ResolveTextLine(strList[sSegID++].c_str(), strSubList, csSubNum, ',');
				sGridID = Corsairs::Util::Str2Int(strSubList[sTCount++]);
				...
				if (iVer == 114) {
					SGridCont.dwDBID = Corsairs::Util::Str2Int(strSubList[sTCount++]);
					SGridCont.sNeedLv = Corsairs::Util::Str2Int(strSubList[sTCount++]);
					SGridCont.bIsLock = Corsairs::Util::Str2Int(strSubList[sTCount++]);
				}
				else {
					SGridCont.dwDBID = 0;
					SGridCont.sNeedLv = 0;
				...
				SGridCont.bItemTradable = Corsairs::Util::Str2Int(strSubList[sTCount++]);
				SGridCont.expiration = Corsairs::Util::Str2Int(strSubList[sTCount++]);

				lnCheckSum += SGridCont.sID + SGridCont.sNum + SGridCont.sEndure[0] + SGridCont.sEndure[1] + SGridCont.
					sEnergy[0] + SGridCont.sEnergy[1] + SGridCont.chForgeLv + SGridCont.dwDBID + SGridCont.sNeedLv +
					SGridCont.bIsLock + SGridCont.bItemTradable;
				...
				if (!bIsOldVer && (Corsairs::Util::Str2Int(strVer[0]) >= 113 || Corsairs::Util::Str2Int(strVer[0]) == 112)) // ???????????????????
				{
	...
	bool SItemGrid2String(std::string& r, /* ???????? */ std::int32_t& lnCheckSum, /* ?????????? */
						  SItemGrid* pGridCont, /* ??? */ int iOrder /* ?????? */) {
		//	???????????????????
	...
	bool String2SItemGrid(SItemGrid* pGridCont, std::int32_t& lnCheckSum, const std::string& sData, int iVer,
						  bool bIsOldVer) {
		//	??????????
		const short csSubNum = 8 + Item::enumITEMDBP_MAXNUM + defITEM_INSTANCE_ATTR_NUM_VER110 * 2 + 1 + 1;
		...
		Corsairs::Util::ResolveTextLine(sData.c_str(), strSubList, csSubNum, ',');

		//	?????????????
		short sTCount = 0;
		short sGridID = Corsairs::Util::Str2Int(strSubList[sTCount++]);
		pGridCont->sID = Corsairs::Util::Str2Int(strSubList[sTCount++]);
		pGridCont->sNum = Corsairs::Util::Str2Int(strSubList[sTCount++]);
		pGridCont->sEndure[0] = Corsairs::Util::Str2Int(strSubList[sTCount++]);
		pGridCont->sEndure[1] = Corsairs::Util::Str2Int(strSubList[sTCount++]);
		pGridCont->sEnergy[0] = Corsairs::Util::Str2Int(strSubList[sTCount++]);
		pGridCont->sEnergy[1] = Corsairs::Util::Str2Int(strSubList[sTCount++]);
		pGridCont->chForgeLv = Corsairs::Util::Str2Int(strSubList[sTCount++]);

		if (iVer == 114) {
			pGridCont->dwDBID = Corsairs::Util::Str2Int(strSubList[sTCount++]);
			pGridCont->sNeedLv = Corsairs::Util::Str2Int(strSubList[sTCount++]);
			pGridCont->bIsLock = Corsairs::Util::Str2Int(strSubList[sTCount++]);
		}
		else {
			pGridCont->dwDBID = 0;
			pGridCont->sNeedLv = 0;
		...
		pGridCont->bItemTradable = Corsairs::Util::Str2Int(strSubList[sTCount++]);
		pGridCont->expiration = Corsairs::Util::Str2Int(strSubList[sTCount++]);
		...
		//	??DB?????
		for (int m = 0; m < Item::enumITEMDBP_MAXNUM; m++) {
			pGridCont->SetDBParam(m, Corsairs::Util::Str2Int(strSubList[sTCount++]));
			lnCheckSum += pGridCont->GetDBParam(m);
		}
		...
		if (!bIsOldVer && (iVer >= 113 || iVer == 112)) // ???????????????????
		{
			...
			{
				for (int k = 0; k < defITEM_INSTANCE_ATTR_NUM; k++) {
					pGridCont->sInstAttr[k][0] = Corsairs::Util::Str2Int(strSubList[sTCount + k * 2]);
					pGridCont->sInstAttr[k][1] = Corsairs::Util::Str2Int(strSubList[sTCount + k * 2 + 1]);
					lnCheckSum += (pGridCont->sInstAttr[k][0] + pGridCont->sInstAttr[k][1]);
				}
			}
			else
	...
	short CKitbag::Regroup(short sSrcPosID, short sSrcNum, short sTarPosID, short sType) {
...

		{
			...
			{
				CItemRecord* pCItem;
				pCItem = Item::GetItemRecordInfo(pSSrcCont.sID);
				if (!pCItem) {
				...
				std::int32_t SrcExpiration = pSSrcCont.expiration;
				std::int32_t TarExpiration = pSTarCont.expiration;
	...
	short CKitbag::Push(SItemGrid* pGrid, short& sPosID, short sType, bool bCommit, bool bSureOpr) {
...

		{
			...
			if (!bSureOpr) {
				CItemRecord* pCItem;
				pCItem = Item::GetItemRecordInfo(pGrid->sID);
				if (!pCItem) {
					sRet = enumKBACT_ERROR_PUSHITEMID;
					goto Error;
				}
...

			{
				...
				if (sPosID >= 0 && sPosID < m_sCapacity) // ??????
				{
					if (bCommit) {
						...
						auto is_acceptable = [&]() -> bool {
							...
							const std::int32_t SrcExpiration = pGrid->expiration;
							const std::int32_t TarExpiration = m_SItem[sType][sPosID].SContent.expiration;
						...
						if (m_SItem[sType][sPosID].SContent.sID == 0)
							sLeftNum -= sPileNum;
						sFreeNum = sPileNum - m_SItem[sType][sPosID].SContent.sNum;
						std::int32_t SrcExpiration = pGrid->expiration;
						std::int32_t TarExpiration = m_SItem[sType][sPosID].SContent.expiration;
						bool SrcTradable = pGrid->bItemTradable;
						bool TarTradable = m_SItem[sType][sPosID].SContent.bItemTradable;
						if (m_SItem[sType][sPosID].SContent.sID == pGrid->sID && (!SrcExpiration && !TarExpiration) && (
							SrcTradable == TarTradable)) {
							if (sFreeNum == 0)
								sPushPos = -1;
							else if (sFreeNum > 0) {
				...
				if (sPileNum > 1) // ?????????????????????????
				{
					for (short i = 0; i < m_sCapacity; i++) {
						...
						std::int32_t TarExpiration = m_SItem[sType][i].SContent.expiration;
						std::int32_t SrcExpiration = pGrid->expiration;
						bool TarTradable = m_SItem[sType][i].SContent.bItemTradable;
						bool SrcTradable = pGrid->bItemTradable;

						if (m_SItem[sType][i].SContent.sID == pGrid->sID && (!SrcExpiration && !TarExpiration) && (
							SrcTradable == TarTradable)) // ???????????
						{
							if (sFreeNum > 0) {
	...
	std::int32_t CKitbag::GetDBParam(short sParamID, short sPosID, short sType) {
		//if (sType < 0 || sType >= enumKBITEM_TYPE_NUM)
		//	return enumKBACT_ERROR_RANGE;
		if (sPosID < 0 || sPosID >= m_sCapacity)
			return enumKBACT_ERROR_RANGE;
		if (sParamID < 0 || sParamID >= Item::enumITEMDBP_MAXNUM)
			return enumKBACT_ERROR_RANGE;
	...
	short CKitbag::SetDBParam(short sParamID, std::int32_t lParamVal, short sPosID, short sType) {
		//if(IsPwdLocked())
	...
	bool CKitbag::CheckValid(void) {
		...
		if (sFactNum != sUseNum) {
			//LG("??????????", "??????????????????????");
			ToLogService("common", "Grid num can't match!");
			return false;
		}
