...
namespace Corsairs::Engine::Render {
	...
	LW_RESULT lwTex::Release() {
		...
		if (_load_type == LOADINGRES_TYPE_RUNTIME_MT) {
			while ((_load_mask & LOADINGRES_MASK_RTMT0) && !(_load_mask & LOADINGRES_MASK_RTMT1)) {
				IThreadPool* tp = _res_mgr->GetThreadPoolMgr()->GetThreadPool(ThreadPoolType::THREAD_POOL_LOADRES);
	...
	lwTex::lwTex(IResourceMgr* res_mgr)
	...
	LW_RESULT lwTex::BeginPass() {
		...
		if (_load_type == LOADINGRES_TYPE_RUNTIME) {
			...
			if (!(_load_mask & LOADINGRES_MASK_RTMT0)) {
				...
				IThreadPoolMgr* tp_mgr = _res_mgr->GetThreadPoolMgr();
				IThreadPool* tp = tp_mgr->GetThreadPool(ThreadPoolType::THREAD_POOL_LOADRES);
	...
	LW_RESULT lwTex::LoadVideoMemory() {
		...
		__load_check_dds:
			...
			if (dds_flag == 1) {
				dds_header = (DDSHeader*)((BYTE*)info->buf->GetData() + sizeof(DWORD));
	...
	lwMesh::lwMesh(IResourceMgr* res_mgr)
	...
	LW_RESULT lwMesh::LoadVideoMemory() {
		...
		IDeviceObject* dev_obj = _res_mgr->GetDeviceObject();
		IStaticStreamMgr* ssm;
		ILockableStreamMgr* lsm;
	...
	ILockableStreamVB* lwMesh::GetLockableStreamVB() {
		ILockableStreamVB* s = 0;

		if (_stream_type != STREAM_LOCKABLE || _vb_id == LW_INVALID_INDEX)
			goto __ret;

		{
			ILockableStreamMgr* lsm = _res_mgr->GetLockableStreamMgr();
	...
	ILockableStreamIB* lwMesh::GetLockableStreamIB() {
		ILockableStreamIB* s = 0;
	...
	lwMtlTexAgent::lwMtlTexAgent(IResourceMgr* mgr)
	...
	lwMeshAgent::lwMeshAgent(IResourceMgr* res_mgr)
	...
	lwResBufMgr::lwResBufMgr(IResourceMgr* res_mgr)
		: _res_mgr(res_mgr) {
	...
	// ThreadPoolMgr
	LW_STD_IMPLEMENTATION(ThreadPoolMgr)
	...
	LW_RESULT ThreadPoolMgr::Create() {
		LW_RESULT ret = LW_RET_FAILED;
		...
		for (DWORD i = 0; i < static_cast<DWORD>(ThreadPoolType::THREAD_POOL_SIZE); i++) {
			...
			for (DWORD j = 0; j < ci[i][0]; j++) {
				DWORD id = ((ThreadPool*)_pool_seq[i])->GetThreadId(j);
	...
	LW_RESULT ThreadPoolMgr::Destroy() {
		LW_RESULT ret = LW_RET_FAILED;
	...
	// ResourceMgr
	LW_STD_IMPLEMENTATION(ResourceMgr)

	ResourceMgr::ResourceMgr(ISysGraphics* sys)
		...
		_static_stream_mgr = LW_NEW(StaticStreamMgr(_dev_obj));
		_dynamic_stream_mgr = LW_NEW(DynamicStreamMgr(_dev_obj));
		_lockable_stream_mgr = LW_NEW(LockableStreamMgr(this));
		_surface_stream_mgr = LW_NEW(lwSurfaceStreamMgr(this));

		_shader_mgr = LW_NEW(ShaderMgr(_dev_obj));
		...
		_thread_pool_mgr = LW_NEW(ThreadPoolMgr());
	...
	// attributes method
	LW_RESULT ResourceMgr::GetAssObjInfo(lwAssObjInfo* info) {
		if (info) {
			*info = _assobj_info;
		}
		return LW_RET_OK;
	...
	LW_RESULT ResourceMgr::SetAssObjInfo(DWORD mask, const lwAssObjInfo* info) {
		if (info) {
	...
	LW_RESULT ResourceMgr::ClearAllMesh() {
	...
	LW_RESULT ResourceMgr::ClearAllTex() {
	...
	LW_RESULT ResourceMgr::ClearAllAnimCtrl() {
	...
	LW_RESULT ResourceMgr::CreateMesh(lwIMesh** ret_obj) {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT ResourceMgr::CreateTex(lwITex** ret_obj) {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT ResourceMgr::CreateAnimCtrl(lwIAnimCtrl** ret_obj, DWORD type) {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT ResourceMgr::CreateAnimCtrlObj(lwIAnimCtrlObj** ret_obj, DWORD type) {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT ResourceMgr::CreateMeshAgent(lwIMeshAgent** ret_obj) {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT ResourceMgr::CreateMtlTexAgent(lwIMtlTexAgent** ret_obj) {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT ResourceMgr::CreateRenderCtrlAgent(lwIRenderCtrlAgent** ret_obj) {
	...
	LW_RESULT ResourceMgr::CreateAnimCtrlAgent(lwIAnimCtrlAgent** ret_obj) {
	...
	LW_RESULT ResourceMgr::CreateRenderCtrlVS(lwIRenderCtrlVS** ret_obj, DWORD type) {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT ResourceMgr::CreatePrimitive(lwIPrimitive** ret_obj) {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT ResourceMgr::CreateHelperObject(IHelperObject** ret_obj) {
		LW_RESULT ret = LW_RET_FAILED;

		HelperObject* o = NULL;

		if ((o = LW_NEW(HelperObject(this))) == NULL)
			goto __ret;
	...
	LW_RESULT ResourceMgr::CreatePhysique(lwPhysique** ret_obj) {
	...
	LW_RESULT ResourceMgr::CreateModel(lwModel** ret_obj) {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT ResourceMgr::CreateItem(lwItem** ret_obj) {
	...
	LW_RESULT ResourceMgr::CreateNode(lwINode** ret_obj, DWORD type) {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT ResourceMgr::CreateNodeObject(lwINodeObject** ret_obj) {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT ResourceMgr::CreateStaticStreamMgr(IStaticStreamMgr** mgr) {
		*mgr = LW_NEW(StaticStreamMgr(_dev_obj));
		return LW_RET_OK;
	}

	LW_RESULT ResourceMgr::CreateDynamicStreamMgr(IDynamicStreamMgr** mgr) {
		*mgr = LW_NEW(DynamicStreamMgr(_dev_obj));
		return LW_RET_OK;
	}

	LW_RESULT ResourceMgr::RegisterMesh(lwIMesh* obj) {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT ResourceMgr::RegisterTex(lwITex* obj) {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT ResourceMgr::RegisterAnimCtrl(lwIAnimCtrl* obj) {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT ResourceMgr::RegisterRenderCtrlProc(DWORD id, lwRenderCtrlVSCreateProc proc) {
		LW_RESULT ret = LW_RET_FAILED;
	...
	//__ret:
	//    return ret;
	LW_RESULT ResourceMgr::QueryTex(DWORD* ret_id, std::string_view file_name) {
	...
	LW_RESULT ResourceMgr::QueryMesh(DWORD* ret_id, const lwResFileMesh* rfm) {
		DWORD found = LW_INVALID_INDEX;
	...
	LW_RESULT ResourceMgr::QueryAnimCtrl(DWORD* ret_id, const lwResFileAnimData* info) {
		DWORD found = LW_INVALID_INDEX;
	...
	LW_RESULT ResourceMgr::UnregisterMesh(lwIMesh* obj) {
		LW_RESULT ret = LW_RET_FAILED;
		lwMesh* o;
	...
	LW_RESULT ResourceMgr::UnregisterTex(lwITex* obj) {
		LW_RESULT ret = LW_RET_FAILED;
		lwITex* o;
	...
	LW_RESULT ResourceMgr::UnregisterAnimCtrl(lwIAnimCtrl* obj) {
		LW_RESULT ret = LW_RET_FAILED;
		lwAnimCtrl* o;
	...
	LW_RESULT ResourceMgr::AddRefMesh(lwIMesh* obj, DWORD ref_cnt) {
		return _pool_mesh.AddRef(obj->GetRegisterID(), ref_cnt);
	}

	LW_RESULT ResourceMgr::AddRefTex(lwITex* obj, DWORD ref_cnt) {
		return _pool_tex.AddRef(obj->GetRegisterID(), ref_cnt);
	}

	LW_RESULT ResourceMgr::AddRefAnimCtrl(lwIAnimCtrl* obj, DWORD ref_cnt) {
		return _pool_animctrl.AddRef(obj->GetRegisterID(), ref_cnt);
	}

	LW_ULONG ResourceMgr::QueryTexRefCnt(lwITex* obj) {
	...
	LW_RESULT ResourceMgr::GetMesh(lwIMesh** ret_obj, DWORD id) {
		LW_RESULT ret = LW_RET_FAILED;
		lwMesh* o;
	...
	LW_RESULT ResourceMgr::GetTex(lwITex** ret_obj, DWORD id) {
		LW_RESULT ret = LW_RET_FAILED;
		lwTex* o;
	...
	LW_RESULT ResourceMgr::GetAnimCtrl(lwIAnimCtrl** ret_obj, DWORD id) {
		LW_RESULT ret = LW_RET_FAILED;
		lwIAnimCtrl* o;
	...
	void ResourceMgr::ReleaseObject() {
	...
	LW_RESULT ResourceMgr::RegisterObject(DWORD* ret_id, void* obj, DWORD type) {
		switch (type) {
		case OBJ_TYPE_MODEL:
	...
	LW_RESULT ResourceMgr::UnregisterObject(void** ret_obj, DWORD id, DWORD type) {
		switch (type) {
		case OBJ_TYPE_MODEL:
	...
	LW_RESULT ResourceMgr::QueryModelObject(void** ret_obj, DWORD model_id) {
	...
	LW_RESULT ResourceMgr::QueryObject(void** ret_obj, DWORD type, std::string_view file_name) {
		switch (type) {
		case OBJ_TYPE_MODEL: {
			lwModel* found = nullptr;
			_pool_model.ForEach([&](DWORD, void* raw) -> bool {
	...
	LW_RESULT ResourceMgr::LoseDevice() {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT ResourceMgr::ResetDevice() {
		LW_RESULT ret = LW_RET_FAILED;
	...
	IDirect3DTextureX* ResourceMgr::getMonochromaticTexture(D3DCOLOR colour, const std::string& filterTexture) {
	...
	IDirect3DTextureX* ResourceMgr::_createMonochromaticTexture(
		D3DCOLOR colour,
	...
	std::string_view ResourceMgr::getTextureOperationDescription(size_t operation) {
