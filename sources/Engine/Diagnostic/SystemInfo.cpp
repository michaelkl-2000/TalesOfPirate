...
namespace Corsairs::Engine::Render {
					...
					if (SUCCEEDED(hr)) {
						...
						if (SUCCEEDED(hr) && var.vt == VT_BSTR && SysStringLen(var.bstrVal) != 0) {
							...
							*pcDirectXVersionLetter = var.bstrVal[0];
							...
							WideCharToMultiByte(CP_UTF8, 0, var.bstrVal, -1, strDestination, 10 * sizeof(CHAR), NULL,
												NULL);
	...
	HRESULT GetDirectXVerionViaFileVersions(DWORD* pdwDirectXVersionMajor,
		...
		if (GetSystemDirectory(szPath, MAX_PATH) != 0) {
			...
			_tcscat(szFile, TEXT("\\ddraw.dll"));
			if (SUCCEEDED(GetFileVersion( szFile, &llFileVersion ))) {
				if ((HIWORD(llFileVersion.HighPart) == 4 && CompareLargeInts(llFileVersion, MakeInt64(4, 8, 0, 400)) >=
						0) || // Win9x version
					(HIWORD(llFileVersion.HighPart) == 5 && CompareLargeInts(llFileVersion, MakeInt64(5, 1, 2258, 400))
			...
			if (SUCCEEDED(GetFileVersion( szFile, &llFileVersion ))) {
				if ((HIWORD(llFileVersion.HighPart) == 4 && CompareLargeInts(llFileVersion, MakeInt64(4, 8, 1, 881)) >=
						0) || // Win9x version
					(HIWORD(llFileVersion.HighPart) == 5 && CompareLargeInts(llFileVersion, MakeInt64(5, 1, 2600, 881))
						>= 0)) // Win2k/WinXP version
				{
				...
				if ((HIWORD(llFileVersion.HighPart) == 4 && CompareLargeInts(llFileVersion, MakeInt64(4, 8, 1, 901)) >=
						0) || // Win9x version
					(HIWORD(llFileVersion.HighPart) == 5 && CompareLargeInts(llFileVersion, MakeInt64(5, 1, 2600, 901))
						>= 0)) // Win2k/WinXP version
				{
			...
			if (SUCCEEDED(GetFileVersion( szFile, &llFileVersion ))) {
				if ((HIWORD(llFileVersion.HighPart) == 4 && CompareLargeInts(llFileVersion, MakeInt64(4, 9, 0, 134)) >=
						0) || // Win9x version
					(HIWORD(llFileVersion.HighPart) == 5 && CompareLargeInts(llFileVersion, MakeInt64(5, 2, 3677, 134))
						>= 0)) // Win2k/WinXP version
				{
	...
	//SystemInfo
	LW_STD_IMPLEMENTATION(SystemInfo)
	...
	LW_RESULT SystemInfo::CheckDirectXVersion() {
	...
	DWORD SystemInfo::GetDirectXVersion() {
		//__ret:
		//    return ret;
