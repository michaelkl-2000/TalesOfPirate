...
namespace Corsairs::Engine::Render {
	...
	LW_STD_IMPLEMENTATION(ShaderMgr9)

	ShaderMgr9::ShaderMgr9(IDeviceObject* dev_obj)
		: _dev_obj(dev_obj), _vs_seq(0), _vs_size(0), _vs_num(0),
		  _decl_seq(0), _decl_size(0), _decl_num(0), _decl_mgr(0) {
	}

	ShaderMgr9::~ShaderMgr9() {
	...
	LW_RESULT ShaderMgr9::Init(DWORD vs_buf_size, DWORD decl_buf_size, DWORD ps_buf_size) {
		_vs_num = 0;
		...
		_decl_mgr = LW_NEW(ShaderDeclMgr(this));

		return LW_RET_OK;
	}

	LW_RESULT ShaderMgr9::RegisterVertexShader(DWORD type, BYTE* data, DWORD size) {
	...
	LW_RESULT ShaderMgr9::RegisterVertexShader(DWORD type, std::string_view file,
	...
	LW_RESULT ShaderMgr9::RegisterVertexDeclaration(DWORD type, D3DVERTEXELEMENT9* data) {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT ShaderMgr9::LoseDevice() {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT ShaderMgr9::ResetDevice() {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT ShaderMgr9::QueryVertexShader(IDirect3DVertexShaderX** ret_obj, DWORD type) {
		LW_RESULT ret = LW_RET_FAILED;
	...
	LW_RESULT ShaderMgr9::QueryVertexDeclaration(IDirect3DVertexDeclarationX** ret_obj, DWORD type) {
		LW_RESULT ret = LW_RET_FAILED;
